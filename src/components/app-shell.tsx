      ...
      {NAV_ITEMS.filter((item) => !item.adminOnly || role === "admin").map((item) => {
        ...
        return (
          <Link
            ...
            className={
              active
                ? "flex items-center gap-2.5 rounded-lg bg-brand-50 px-3 py-2 text-sm font-medium text-brand-700"
                : "flex items-center gap-2.5 rounded-lg px-3 py-2 text-sm font-medium text-ink-500 hover:bg-ink-50 hover:text-ink-900"
            }
          >
    ...
    <div className="flex min-h-screen bg-ink-50/50">
      ...
      <aside className="hidden w-60 shrink-0 flex-col border-r border-ink-100 bg-white md:flex">
        ...
        <div className="border-t border-ink-100 p-3">
          <div className="mb-2 truncate px-2 text-xs text-ink-400">{fullName}</div>
          <form action={signOut}>
